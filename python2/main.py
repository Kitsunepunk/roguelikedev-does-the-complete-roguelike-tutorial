    ...
    def clear(self):
        if libtcod.map_is_in_fov(fov_map, self.x, self.y):
            ...
            if x is not None:
                ...
                libtcod.console_put_char_ex(mapcon, x, y, '.', color_light_ground, libtcod.black)
...
def move_camera(target_x, target_y):
    ...
    # new camera coordinates (top-left corner of the screen releative to the map)
    x = target_x - CAM_WIDTH / 2
    y = target_y - CAM_HEIGHT / 2

    if x < 0: x = 0
    if y < 0: y = 0
    if x > MAP_WIDTH - CAM_WIDTH - 1: x = MAP_WIDTH - CAM_WIDTH - 1
    if y > MAP_HEIGHT - CAM_HEIGHT - 1: y = MAP_HEIGHT - CAM_HEIGHT - 1
    ...
    if fov_recompute:
        ...
        for y in range(CAM_HEIGHT):
            for x in range(CAM_WIDTH):
                ...
                if not visible:
                    libtcod.console_put_char_ex(mapcon, x, y, 178, libtcod.darkest_grey, libtcod.azure)
                    ...
                    #it's visible
                    if wall:
                        libtcod.console_put_char_ex(mapcon, x, y, '#', color_light_wall, libtcod.black )
                    else:
                        libtcod.console_put_char_ex(mapcon, x, y, '.', color_light_ground, libtcod.black )
...
libtcod.console_set_custom_font('cp437_12x12.png', libtcod.FONT_TYPE_GREYSCALE | libtcod.FONT_LAYOUT_ASCII_INROW)
